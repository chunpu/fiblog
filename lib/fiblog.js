...
fiblog.start = function(blogpath) {
  ...
  $.lastUpdate = new Date();
	...
	http.createServer(function(req, res) {
		...
		if (patharr[0] === 'cmd') {
			// pre cmd
			require('./cmd.js')(req, res, function(err) {
        // update blog hash
	      files = fileList(blogpath);
	      $.blog = genBlogHash(files, blogpath);
      });
			return;
		}
			...
			fs.readFile(filePath, function(err,data) {
				...
				if (err) {
					...
					res.writeHead(200,{
						...
						'Cache-Control': 'max-age='+0.5*24*60*60,					
						'Expires'	   : new Date(new Date().getTime()+0.5*24*60*60*1000).toGMTString()
					});
					res.end(data);
				}
			})
...
/*
function checkFileChange(mypath) {
  // error
  console.log(mypath);
  fs.lstat(mypath, function(err, stats) {
    console.log(stats);
    console.log('mtime: '+stats.mtime);
    console.log('lastUpdate: '+$.lastUpdate);
    if (stats.mtime > $.lastUpdate) {
      // means blogHash need to be updated
      console.log('Update');
	    $.blog = genBlogHash(files, blogpath);
    }
  });
